...
@Directive({
    ...
    providers: [
        { provide: NG_VALIDATORS, useExisting: forwardRef(() => DateTimeValidatorDirective), multi: true }
    ]
})
...
export class DateTimeValidatorDirective implements Validator {
    @Input() fromDate: Date;
    @Input() toDate: Date;
    constructor(@Attribute('dateTimeValidator') public dateTimeValidator: string, private _element: ElementRef) {}

    validate(c: AbstractControl): { [key: string]: any } {
        const useSeconds = this._element.nativeElement.getAttribute('data-seconds') === 'true';
        // Use value from input field unless c.value exists (it's a Date object)
        const inputValue = c.value ? momentInstance(c.value).
            format(useSeconds ? DATE_FORMAT.YEAR_MONTH_DAY_HOUR_MINUTE_SECOND : DATE_FORMAT.YEAR_MONTH_DAY_HOUR_MINUTE)
            : (this._element.nativeElement && this._element.nativeElement.querySelector('.cui-input') ?
            this._element.nativeElement.querySelector('.cui-input').value : '');

        let isValid = DateHelper.isValidDateTime(inputValue, useSeconds);
